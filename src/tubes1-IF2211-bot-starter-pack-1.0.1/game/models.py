...
class Board:
    ...
    def bots(self) -> List[GameObject]:
        return [d for d in self.game_objects if d.type == "BotGameObject"] # List of bots
    ...
    def is_valid_move(
        ...
        if not (-1 <= delta_x <= 1) or not (-1 <= delta_y <= 1):   # Check if delta values are between -1 and 1 inclusive
            print(
        ...
        if delta_x == delta_y:
            print(
                Fore.RED + Style.BRIGHT + "Invalid move:" + Style.RESET_ALL,    # Check if delta_x and delta_y are equal: GABISA DIAGONAL
            )
            return False

        if not (0 <= current_position.x + delta_x < self.width):  # Check if the new position is within the board
            print(
                Fore.RED + Style.BRIGHT + "Invalid move:" + Style.RESET_ALL,
        ...
        if not (0 <= current_position.y + delta_y < self.height):   # Check if the new position is within the board
            print(
                Fore.RED + Style.BRIGHT + "Invalid move:" + Style.RESET_ALL,
